...
iree_uk_mmt4d_tile_s8s4s32_2x8x16_to_8x8x16_arm_64_i8mm(
  ...
  for (int k = 0; k < params->K; k++) {
    ...
    IREE_UK_UNROLL for (int m = 0; m < miter; m++) {
      int8x16_t lhs[2][2];
      if (M0 > 2) {
        IREE_UK_UNROLL for (int i = 0; i < 2; i++) {
          lhs[0][i] = vld1q_s8(lhs_ptr + 16 * 2 * i);
          lhs[1][i] = vld1q_s8(lhs_ptr + 16 * (2 * i + 1));
...
//          lhs[1][i] = vcombine_s8(lhs_0.val[1], lhs_1.val[1]);
        ...
        lhs[0][0] = vld1q_s8(lhs_ptr);
        lhs[1][0] = vld1q_s8(lhs_ptr + 16);
