...
iree_uk_mmt4d_tile_s8s4s32_1x8x16_to_2x8x16_arm_64_i8mm(
    ...
    const void* IREE_UK_RESTRICT rhs_panel,
    const iree_uk_mmt4d_params_t* params, int M0) {
  IREE_UK_ASSERT(M0 >= 1 && M0 <= 2 && iree_uk_is_po2_u32(M0));
  IREE_UK_ASSERT(!(params->K0 % 16));
  const iree_uk_int8_t* IREE_UK_RESTRICT lhs_ptr = lhs_panel;
  const iree_uk_int8_t* IREE_UK_RESTRICT rhs_ptr = rhs_panel;
  iree_uk_int32_t* IREE_UK_RESTRICT out_ptr = out_tile;
  ...
  IREE_UK_UNROLL for (int j = 0; j < 2; j++) {
    ...
    if (M0 > 1) {
      int32x4_t acc_1x4_1 =
          iree_uk_neon_uzp2_s32_as_s64(acc[2 * j + 0], acc[2 * j + 1]);
      if (params->flags & IREE_UK_FLAG_MMT4D_ACCUMULATE) {
        int32x4_t existing_acc = vld1q_s32(out_ptr + 8 + 4 * j);
        acc_1x4_1 = vaddq_s32(acc_1x4_1, existing_acc);
      }
      vst1q_s32(out_ptr + 8 + 4 * j, acc_1x4_1);
    }
  }
}
...
IREE_UK_MMT4D_TILE_FUNC_IMPL_FOR_M0(
    iree_uk_mmt4d_tile_s8s4s32_1x8x16_to_2x8x16_arm_64_i8mm,
    iree_uk_mmt4d_tile_s8s4s32_1x8x16_arm_64_i8mm, 1)
IREE_UK_MMT4D_TILE_FUNC_IMPL_FOR_M0(
    iree_uk_mmt4d_tile_s8s4s32_1x8x16_to_2x8x16_arm_64_i8mm,
